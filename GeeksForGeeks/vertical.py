...
def verticalOrder(root): 
    #Your code here
    dist_dict = {}
    buildHorizontalDistance(root, 0, dist_dict)
    for key in sorted(dist_dict):
        print(*dist_dict[key], end = " ")
    # code here
def buildHorizontalDistance(root, dist, dist_dict):
    if dist in dist_dict:
        dist_dict[dist].extend([root.data])
    else:
        dist_dict[dist] = [root.data]
    if(root.left):
        buildHorizontalDistance(root.left,dist-1,dist_dict)
    if(root.right):
        buildHorizontalDistance(root.right, dist+1,dist_dict)